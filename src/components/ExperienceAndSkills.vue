    ...
    <div class="md:grid md:grid-cols-2 gap-8 items-center py-8 px-4 xl:gap-16 xl:px-16">
      <div class="mt-4 md:mt-0 text-left flex flex-col z-10 h-full w-[80%]">
        ...
        <div class="mt-8" v-for="skill in Skills" :key="skill.id">
          ...
          <div class="mt-2 h-1 w-full bg-tertiary rounded-full">
            <div class="h-1 rounded-full bg-primary" :style="`width :${skill.width}`"></div>
          </div>
        </div>
      </div>
      <div data-aos="flip-left">
        <h2 class="text-4xl font-bold text-white text-left mb-4 md:text-center md:mt-0 mt-8">My Experiences</h2>
        <div class="space-y-8 py-8" data-aos="fade-left">
          <div v-for="element in Experience" :key="element.id" class="flex items-center rounded-xl p-4 bg-tertiary shadow-lg border border-secondary">
            <div class="w-1/4">
